...
const INPUTS = {
  ...
  BACKUP_S3_PREFIX: "d1/acme-ops-production",
};
...
function writeStubs({ dir, log, state, world }) {
  ...
  const initial =
    world === "full"
      ? {
          databases: [
            { uuid: STAGING_DB_ID, name: "acme-ops-staging" },
            { uuid: PRODUCTION_DB_ID, name: "acme-ops-production" },
          ],
...
test("--plan reports every step, performs no mutating call and redacts every secret", () => {
  ...
  for (const expected of [
    "[would create] database acme-ops-staging",
    "[would create] database acme-ops-production",
    "[would create] Worker acme-ops-staging",
  ...
  assert.match(
    run.stdout,
    /\$ wrangler d1 create acme-ops-staging --jurisdiction eu/,
  );
...
test("--yes issues exactly the expected argument arrays and stdin bodies", () => {
  ...
  for (const expected of [
    ...
    ">>> wrangler [d1] [create] [acme-ops-staging] [--jurisdiction] [eu]",
    ">>> wrangler [d1] [create] [acme-ops-production] [--jurisdiction] [eu]",
  ...
  assert.ok(written.includes('"database_name": "acme-ops-local"'));
