...
if (!base || !password) throw new Error("STAGING_URL and SEED_PASSWORD are required");
...
async function call(label, path, { method = "GET", body, ms = 20000 } = {}) {
  ...
  try {
    ...
    console.log(`${label.padEnd(30)} ${String(r.status).padEnd(12)} ${String(Date.now() - started).padStart(6)}ms  colo=${ray.split("-")[1] ?? "?"}  ray=${ray}  ${text.replace(/\s+/g, " ").slice(0, 90)}`);
    try { return JSON.parse(text); } catch { return undefined; }
  } catch (error) {
    console.log(`${label.padEnd(30)} ${"NO RESPONSE".padEnd(12)} ${String(Date.now() - started).padStart(6)}ms  ${error.name}`);
    return undefined;
  }
}
...
await call("POST create-job (atomicBatch)", "/_agent-native/actions/create-job", {
  method: "POST",
  body: { customerId: "cus_a", title: `Diag ${runId}`, scheduledAt: "2030-01-15T09:00:00.000Z", idempotencyKey: runId },
});
// Again, with a longer ceiling: does it ever finish, or never?
await call("POST create-job (60s ceiling)", "/_agent-native/actions/create-job", {
  method: "POST",
  ms: 60000,
  body: { customerId: "cus_a", title: `Diag ${runId}-b`, scheduledAt: "2030-01-15T09:00:00.000Z", idempotencyKey: `${runId}-b` },
});
console.log("\nIf the writes never answer but login did, the difference is the batch write, not POST or auth.");
console.log("Compare the colo above with a run of the same probe from a developer machine.");
